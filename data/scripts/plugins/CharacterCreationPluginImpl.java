...
//	private ResponseImpl SOMETHING_ELSE_2 = new ResponseImpl("Did something else");
        ...
        private ResponseImpl KADUR = new ResponseImpl("Kadur.");
        ...
        private ResponseImpl INS = new ResponseImpl("It's not working when you fight against the Kadur oppression.");
        ...
        private ResponseImpl KAD = new ResponseImpl("The Theocracy is generous towards those who believe.");
        ...
        private ResponseImpl NOM_F = new ResponseImpl("Corvussian, I earn a living for the Armada, scouting this system.");
        private ResponseImpl INS_F = new ResponseImpl("We have to do with what we find. *You point your gun at the merchant's face*");
	...
	public void submit(Response response, CharacterCreationData data) {
		...
		if (stage == 0) { // just in case
			if (Global.getSettings().isDevMode() && true) {
				...
				data.getStartingCargo().addCrew(CrewXPLevel.REGULAR, 2000);
		...
		if (response == GNEISS) {
			...
			data.getStartingCargo().addCrew(CrewXPLevel.REGULAR, 20);
                ...
                } else if (response == KAD) {
                        data.addStartingFleetMember("khs_jackal_Scout", FleetMemberType.SHIP);
                        data.getStartingCargo().addFuel(20);
			data.getStartingCargo().addSupplies(5);
			data.getStartingCargo().addCrew(CrewXPLevel.REGULAR, 4);
                        UsSData.player_id = "Voice of Kadur";
                } else if (response == INS_F) {
                        data.addStartingFleetMember("lasher_Standard", FleetMemberType.SHIP);
                        data.getStartingCargo().addFuel(20);
			data.getStartingCargo().addSupplies(20);
			data.getStartingCargo().addCrew(CrewXPLevel.REGULAR, 50);
                        data.getStartingCargo().getCredits().add(7000f);
                        UsSData.player_id = "Voice of Qamar";
		}
	}
	...
	public void startingShipPicked(String variantId, CharacterCreationData data) {
		...
		if (variantId.equals("pack_bedlington_Standard")) {
			...
			data.getStartingCargo().addCrew(CrewXPLevel.VETERAN, 19);
                ...
                if (variantId.equals("khs_hyena_Picket")) {
			data.getStartingCargo().addFuel(45);
			data.getStartingCargo().addSupplies(10);
			data.getStartingCargo().addCrew(CrewXPLevel.ELITE, 1);
			data.getStartingCargo().addCrew(CrewXPLevel.VETERAN, 23);
		}
	}
