...
public class ReflectorAI implements ShipSystemAIScript
{
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target)
    {
                ...
                iter.hasNext();) //iterate through every projectile on the map
        {
            ...
            if ((proj.getOwner() != ship.getOwner()) && (MathUtils.getDistance(proj, proj) >= (field_range * 1.05f))) //here we're doing two checks: is the projectile hostile? Is it in range? Notice that we're also counting projectiles just outside the distance so we can predict them
            {
